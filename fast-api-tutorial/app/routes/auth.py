...
async def is_email_exist(email: str) -> Boolean:
...
async def register(sns_type: SnsType, reg_info: UserRegister, session: Session = Depends(db.session)):
    ...
    if sns_type == SnsType.email:
        ...
        token = dict(Authorization=f"Bearer {create_access_token(data=UsersToken.from_orm(new_user).dict(exclude={"pw", "marketing_agree")
        return token
