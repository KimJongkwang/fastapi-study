...
### 16장 FastAPI 테스트 코드 작성하기(프로가 되는 길)

- 프로와 아마추어를 넘는 길
- 파이썬, 자바스크립트와 같은 인터프리터 언어에서는 특히 사이드 이펙트를 잡을 수 있는 유일한 방법이라 해도 과언이 아님

- 테스트 프로세스?
  - 테스트 코드 작성
  - 데이터베이스 생성
  - 테스트 이후 데이터베이스 삭제
  - 테스트 코드가 나의 코드의 커버리지를 확인
- 일반적으로 위와 같은 방법은 있겠지만, 본인의 경험에서 스스로 나아가는 것
...
- pytest를 활용하여 테스트코드
  ...
  - @pytest.fixture(scope="session") # fixture: 하나의 도구처럼 사용됨

### 17장 Git 커밋으로 바로 서버 반영, CICD의 마법 X FastAPI
...
- CICD Pipeline
  - 1. Elastic Beanstalk 생성
  - 2. Database (AWS RDS) 생성.
  - 3. config.py 수정
  - 4. Gunicorn.conf.py 추가
  - 5. Dockerfile 추가
  - 6. httptool uvloop gunicorn 설치
  - 7. .travis.yml 파일 추가
  - 8. travis 가입
  - 9. travis 설정
  - 10. 배포
