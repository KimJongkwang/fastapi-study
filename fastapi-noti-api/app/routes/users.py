...
async def get_me(request: Request):
    ...
    user_info = Users.filter(id__gt=user.id).order_by("email").count()  # django orm style
    # user_info = session.query(Users).filter(Users.id > user.id).order_by(Users.email.asc()).count()  # sql alchemy orm style
    return user_info
